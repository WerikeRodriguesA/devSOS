...
/**
 * Autenticação: cria conta e faz login (emite o JWT).
 *
 * <p>Ambas as rotas são PÚBLICAS (definido no {@code SecurityConfig}) — o
 * cliente não tem token ainda quando está se registrando ou logando.</p>
 */
@RestController
...
public class AuthController {
    ...
    /** POST /api/auth/register — cria a conta e devolve o token de acesso. */
    @PostMapping("/register")
    public ResponseEntity<AuthResponseDTO> registrar(@Valid @RequestBody RegistroRequestDTO request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(authService.registrar(request));
    }

    /** POST /api/auth/login — valida credenciais e devolve o token de acesso. */
    @PostMapping("/login")
