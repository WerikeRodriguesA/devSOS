-- ============================================================================
-- devSOS — V1: schema inicial (PostgreSQL 13+)
-- Aplicada pelo Flyway no boot do backend. Espelha database/schema.sql
-- (mantida aqui como migração versionada; aquele arquivo vira referência
-- documental do estado "final").
-- Nota: sem BEGIN/COMMIT — o Flyway roda cada migração em transação própria.
--
-- ESCOPO
--   1. users          : cadastro dos devs (quem pede e quem ajuda)
--   2. posts          : os "problemas" publicados no feed
--   3. sessions       : a "corrida" — vínculo entre post, helper e chat
--   4. reviews        : avaliação mútua após uma sessão concluída
--   5. chat_messages  : mensagens trocadas na sala de uma corrida
-- ============================================================================
